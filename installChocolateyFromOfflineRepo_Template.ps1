...
$packageRepo = 'http://YOUR_REPO/chocolatey'
...
# UPDATE THIS PATH
$localChocolateyPackageFilePath = 'c:\packages'
$localChocolateyPackageFile = 'c:\packages\chocolatey.nupkg'
...
$unzipMethod = 'builtin'
#$unzipMethod = '7zip'
#$7zipUrl = '\\YOUR_FILE_SHARE\7za.exe' #(download this file, host internally, and update this to internal)
...
#$env:ChocolateyEnvironmentDebug = 'true'
  ...
  if (!(Test-Path($localChocolateyPackageFilePath))) {
    mkdir -Path "$localChocolateyPackageFilePath" -Force
    }
...
if (!(Test-Path $ChocoInstallPath)) {
  ...
  if ($searchUrl) {
    Download-Package $searchUrl $localChocolateyPackageFile
  }

  # Install Chocolatey
  Install-LocalChocolateyPackage $localChocolateyPackageFile
}

choco source add -n="YOUR_REPO" -s="http://YOUR_REPO/chocolatey" -u="YOUR_USER" -p="YOUR_PASSWORD"
